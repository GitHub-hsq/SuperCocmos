...
export async function getProviders(req: Request, res: Response) {
  ...
  try {
    ...
    // 尝试从缓存获取
    let providers = await measurePerformance(
      'Redis缓存读取',
      async () => await getCached(cacheKey),
      200, // 超过200ms视为慢操作
    )

    if (!providers) {
      // 缓存未命中，从数据库查询
      providers = await measurePerformance(
        'Supabase查询 (providers_with_models)',
        async () => await getAllProvidersWithModels(),
        1000, // 超过1秒视为慢操作
      )

      // 保存到缓存（异步，不阻塞响应）
      setCached(cacheKey, providers, CACHE_TTL.PROVIDER_LIST).catch((err) => {
        logger.error(`缓存写入失败: ${err.message}`)
      })
    }
    // 缓存命中时不输出日志，减少冗余
