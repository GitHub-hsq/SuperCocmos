...
const enabledModels = computed(() => {
  return modelStore.enabledModels.filter((model: any) => model.enabled !== false)
})

// 模型选项（只显示已启用的模型）
const modelOptions = computed(() => {
  return enabledModels.value.map((model: any) => ({
...
// 更新节点配置
function updateNodeConfig(nodeType: Model.WorkflowNodeType, key: keyof Model.ModelConfig, value: number | null) {
  if (value === null)
    return
