...
// 移动端收起时宽度为 0，非移动端保持 55px
const collapsedWidth = computed(() => {
  return isMobile.value ? 0 : 55
})
            ...
            <div class="sider-header nav-bg" :class="{ collapsed }">
              ...
              <template v-if="!collapsed">
                <div class="sider-logo-wrapper">
                  <SuperCocmosIcon :icon-color="isDark ? '#fff' : '#161618'" />
                  <span class="sider-logo-text">SuperCocmos</span>
                </div>
                <button
                  class="sider-toggle-btn"
                  @click="(workMode === 'novel' || workMode === 'quiz') ? handleBackToChat() : handleUpdateCollapsed()"
                >
                  <!-- 小说模式或题目模式：返回箭头 -->
                  <SvgIcon v-if="workMode === 'novel' || workMode === 'quiz'" icon="ri:arrow-left-line" class="text-xl" />
                  <!-- 聊天模式：折叠图标 -->
              ...
              <template v-else>
                <div class="sider-collapsed-logo-wrapper" @click="handleUpdateCollapsed">
                  <SuperCocmosIcon class="sider-collapsed-logo-icon" :icon-color="isDark ? '#fff' : '#161618'" />
...
.dark .sider-toggle-btn {
  color: #fff;
}

.dark .sider-toggle-btn svg {
  color: #c9d1d9;
}

.dark .sider-toggle-btn:hover {
  background: rgba(255, 255, 255, 0.05);
  color: #fff;
}

.dark .sider-toggle-btn:hover svg {
  color: #fff;
}

.dark .sider-logo-text {
  color: #fff;
}

.dark .sider-logo-icon {
  color: #fff;
}

.dark .sider-collapsed-expand-btn {
  color: #fff;
}

.dark .sider-collapsed-expand-btn svg {
  color: #fff;
}
...
.dark .sider-icon-btn:hover {
  background: rgba(255, 255, 255, 0.05);
  color: #fff;
}

/* 深色模式下的按钮样式 */
.dark .n-button--default-type {
  background-color: transparent;
  color: #c9d1d9;
}

.dark .n-button--default-type:hover {
